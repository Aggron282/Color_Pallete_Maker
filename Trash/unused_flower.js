...
class Root {
  ...
  constructor(x,y){
      ...
      this.y = x;
      console.log(x,y)
      this.speedX = Math.random() * 27 -2;
      this.speedY = Math.random() * 27 -2;
      this.maxSize =Math.random() * 5 + 1;
      ...
      this.willGrow = this.maxSize >= 0 ? true : false;
    }
      update(){
...
class Flower {
  ...
  constructor(x,y,size){
   ...
   var random_flowers = ["blue_1","blue_2","blue_3","red_1","red_2","red_3"];
  // var random_flowers = ["red_1","r","9","10"]
    ...
    this.maxFlowerSize = this.size * Math.random() *  60 + 10
  ...
  grow(){
    ...
    if(this.size < this.maxFlowerSize){
      this.size += this.maxFlowerSize / 5;
      this.angle += this.va;
      csv.save();
